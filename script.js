...
function updateEventPetSelect(selectedId = "") {
    ...
    if (!select) return;
    select.innerHTML = '<option value="">Для всех питомцев / Общее</option>';
    pets.forEach((pet, idx) => {
        const opt = document.createElement("option");
        opt.value = String(idx);
        opt.textContent = pet.name;
        if (String(idx) === selectedId) opt.selected = true;
...
function openEventModal(date = todayISO(), index = null) {
    ...
    if (index === null) {
        eventIndex.value = ""; eventDate.value = date; eventType.value = "vaccine"; eventTitle.value = "";
        updateEventPetSelect("");
        title.textContent = "Добавить событие";
    } else {
        const item = events[index]; eventIndex.value = String(index); eventDate.value = item.date; eventType.value = item.type; eventTitle.value = item.title;
        updateEventPetSelect(item.petId !== undefined ? String(item.petId) : "");
...
function saveEvent() {
    const eventIndex = document.getElementById("eventIndex"), eventDate = document.getElementById("eventDate"), eventType = document.getElementById("eventType"), eventTitle = document.getElementById("eventTitle"), eventPetId = document.getElementById("eventPetId");
    const date = eventDate.value, type = eventType.value, title = eventTitle.value.trim();
    const petId = eventPetId ? (eventPetId.value === "" ? null : Number(eventPetId.value)) : null;
    if (!date) { alert("Выбери дату."); return; }
    if (!title) { alert("Напиши описание события."); return; }
    const item = { id: Date.now() + Math.random(), date, type, title, petId };
    const index = eventIndex.value === "" ? null : Number(eventIndex.value);
    if (index === null) events.push(item); else events[index] = { ...events[index], date, type, title, petId };
...
function renderUpcomingEvents() {
    ...
    container.innerHTML = upcoming.map(ev => {
        const petBadge = ev.petId !== undefined && pets[ev.petId] ? `<span style="background:#e0e7ff;color:#3730a3;padding:2px 8px;border-radius:12px;font-size:12px;margin-left:6px;">${escapeHtml(pets[ev.petId].name)}</span>` : '';
        return `<div class="event-card"><div class="event-main"><div class="event-title">${escapeHtml(eventTypeLabel(ev.type))}</div><div class="small">${formatDate(ev.date)} — ${escapeHtml(ev.title)} ${petBadge} <span style="color:#22c55e;font-weight:600;margin-left:6px;">(через ${ev.daysLeft} дн.)</span></div></div>${!ev.isAuto ? `<div class="record-actions"><button class="btn btn-light" onclick="editEvent(${ev.index})">✏️</button><button class="btn btn-danger" onclick="deleteEvent(${ev.index})">🗑️</button></div>` : ''}</div>`;
    }).join("");
}
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        dayEvents.slice(0, 2).forEach(ev => {
            const petName = ev.petId !== undefined && pets[ev.petId] ? ` (${pets[ev.petId].name})` : '';
            const p = document.createElement("div"); p.className = `event-pill ${ev.type}`; p.textContent = ev.title + petName; cell.appendChild(p);
        });
...
function renderEventList() {
    ...
    container.innerHTML = sorted.map((ev, idx) => {
        const petName = ev.petId !== undefined && pets[ev.petId] ? `<span style="background:#e0e7ff;color:#3730a3;padding:2px 8px;border-radius:12px;font-size:12px;margin-left:8px;">${escapeHtml(pets[ev.petId].name)}</span>` : '';
